...
export default function AdminEditCompetitionPage() {
  ...
  useEffect(() => {
    async function loadCompetition() {
      try {
        ...
        const competition: Competition = await res.json()
        ...
        setTags(competition.tags || [])
        setStatus(competition.status)
        
        // Convert existing images/videos to UploadedMedia format
        setImageUploads(
          (competition.images || []).map((url, idx) => ({
            id: `existing-img-${idx}`,
            url,
            name: `Image ${idx + 1}`,
          }))
        ...
        setVideoUploads(
          (competition.videos || []).map((url, idx) => ({
            id: `existing-vid-${idx}`,
            url,
            name: `Video ${idx + 1}`,
          }))
  ...
  function validate() {
    ...
    if (deadline && status !== 'closed' && new Date(deadline) <= new Date()) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const message = data?.error ?? 'Failed to update competition'
        throw new Error(message)
      }

      toast.success('Competition updated successfully ❄️')
      router.push('/admin/competitions')
      router.refresh()
    } catch (error: any) {
      console.error(error)
      toast.error(error.message || 'Something went wrong while updating competition')
      setState('idle')
    }
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen p-8 flex items-center justify-center" style={{ backgroundColor: 'var(--background)' }}>
        <div className="text-center">
          <div className="text-4xl mb-4">❄️</div>
          <p style={{ color: 'var(--color-text-primary)' }}>Loading competition...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen p-8" style={{ backgroundColor: 'var(--background)' }}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          >
            Update the competition details. Changes will be reflected immediately.
          </p>
        </div>
        ...
        >
          ...
          <div>
            ...
            <div className="flex gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  style={{ accentColor: 'var(--color-glacier-500)' }}
                />
                <span style={{ color: 'var(--color-text-primary)' }}>Closed</span>
              </label>
            </div>
            <p className="mt-2 text-xs" style={{ color: 'var(--color-text-muted)' }}>
              {status === 'draft' 
                ...
                : 'Competition is closed and no longer accepting registrations'}
            </p>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isSubmitting ? 'Updating... ❄️' : 'Update Competition ❄️'}
            </button>
            <button
              type="button"
              onClick={() => router.back()}
